/* Fluid Glass Material Engine — adapted from fluidglass-ui/references/fluid-glass-engine.js */
/* Dashboard-only: renders cards into #fluidGlassGrid; no settings panel. */
(() => {
  'use strict';

  const CANONICAL_STORAGE_KEY = 'knowledge_os_fluid_glass_config';
  ...
  const DEFAULTS = { storageKey: CANONICAL_STORAGE_KEY, quality: 'auto', interaction: true, cards: defaultCards(4) };
  ...
  function loadSettings(seedConfig) {
    ...
    try { const saved = localStorage.getItem(base.storageKey); return saved ? mergeSettings({ ...base, ...JSON.parse(saved) }) : base; }
    catch (_) { return base; }
  }
  ...
  class FluidGlassRenderer {
    constructor(element, config, index, manager) {
      this.element = element; this.key = config.key; this.index = index; this.manager = manager;
      this.canvas = element.querySelector('.fg-fluid-canvas');
      this.config = config; this.gl = null; this.program = null; this.buffer = null; this.uniforms = {};
      this.active = true; this.failed = false; this.lastFrame = 0;
    ...
    init() {
      ...
      try {
        ...
        this.gl = gl; liveContexts++;
        gl.clearColor(0, 0, 0, 0);
        this.program = buildProgram(gl); gl.useProgram(this.program);
        const buffer = gl.createBuffer(); this.buffer = buffer;
        ...
        this.failed = false; this.element.classList.remove('is-fallback'); this.resize(); return true;
      } catch (error) { console.warn(`[FluidGlass:${this.key}]`, error); this.fallback(error.message); return false; }
    }
  ...
  class FluidGlassManager {
    constructor() {
      ...
      this.renderers = []; this.running = true;
      document.querySelectorAll('[data-fluid-glass-key]').forEach((element, index) => {
        ...
        this.renderers.push(renderer); renderer.init();
      });
    ...
    setQuality(value) { settings.quality = value; this.profile = qualityProfile(value); this.renderers.forEach((renderer) => renderer.setProfile()); }
  ...
  function render() {
    ...
    window.FluidGlassMaterial = {
      ...
      setQuality: (value) => { if (QUALITY_VALUES.includes(value)) { manager.setQuality(value); } },
      setColors: (key, a, b, c) => { const card = settings.cards.find((item) => item.key === key); if (card) { card.a = safeHex(a, card.a); card.b = safeHex(b, card.b); card.c = safeHex(c, card.c); card.preset = 'custom'; manager.refresh(key); } },
