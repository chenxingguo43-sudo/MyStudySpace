...
9. ✅ **语法增强（缩小版）**（2026-08-29 完成）：**虚拟滚动连续练习移植**——新增 `isB2VirtualQuizBook()`（A 语法书 + B2 语法模块统一判断），`renderQuizChapter` 的虚拟容器/初始化/滚动窗口/恢复定位/jump 共 9 处门禁扩展到 B2 语法；实测 P1（60 题）只渲染 20 行窗口、滚动到底 top spacer 17939px 且窗口正确移到 P1-Q060。**薄弱知识点面板**——`renderB2WeakKnowledgePanel`：按本部分历史错题聚合到知识点（错 N 题 · 待掌握/已纠正），点击直达该点练习（`restartQuiz('knowledge')`），琥珀色面板样式与 A 的 `zlatoust-weak-panel` 同构，接入 B2 quiz 页。教学页/理论卡内容层仍由用户制卡 skill 独立建设（决策 6）。
10. ✅ **错题本补收 B2 真题 + 标注范围**（决策 5，2026-08-29 完成）：`answerExamQuestion` 记录补 `everWrong/lastResult`；`getWrongAnswerItems` 在错题本真题索引（`wrongAnswerExamMeta`：questionId→分测验序号/标题，进入错题本时按清单构建，实测 25 题）建立时纳入真题错题；错题行"真题 · 分测验名"点击 `openExamWrongAnswerItem` 跳转对应分测验并滚动定位到该题（题目节点补 data-question-id）；页头标注"仅含俄语 B2 的客观题错题：语法 P1–P6、阅读、真题模拟"，筛选下拉补阅读/真题选项。A 的错题收纳仍为另立任务（本次不做）。
