...
├── scripts/
│   ├── health_check.js   # 健康检查脚本（零 LLM 调用，当前推荐）
│   └── health_check.py   # 旧版健康检查脚本（需要 Python）
└── CLAUDE.md
```
...
| `/health` | 说 "health" 或 "健康检查" | 运行 `node scripts/health_check.js`，检测空文件/断链/缺失字段/标签规范 |

### `/ingest` 工作流

1. 读取 `raw/<文件名>`，识别类型（对话/文章/词汇表/语法讲解）
2. 提取生词 → 判断词性（noun/verb/adj/adv/particle）→ 按 `模板/生词模板.md` 创建到 `词汇/{词性}/`
3. 提取句型 → 归档到 `B2口语素材/<章节>/`，自动推断 section
4. 识别语法点 → 归档到 `语法/`
5. **更新对应词性的索引**（`词汇/{词性}/{词性}索引.md`）
6. **同步更新导入文件**（`{词性}导入.txt`，格式 `俄语, 中文释义`）
7. 追加操作记录到 `wiki/log.md`

### 导入文件

词汇根目录下的 `{词性}导入.txt` 用于批量导入背单词软件，每次摄入新词后自动同步：

| 文件 | 内容 | 格式 |
|------|------|------|
| `名词导入.txt` | 276 词 | `俄语, 中文释义` |
| `动词导入.txt` | 155 词 | 同上 |
| `形容词导入.txt` | 74 词 | 同上 |
| `副词导入.txt` | 9 词 | 同上 |
| `功能词导入.txt` | 2 词 | 同上 |
...
B2高频词（91 个）已接入 Anki：
- **Obsidian 端**：B2高频词/*.md 文件末尾有 `#flashcard` + `::` 分隔符
- **推送**：`Ctrl+P` → `Flashcards: Generate for current file`（逐文件）
- **批量导入**：`python scripts/batch_import.py`（通过 AnkiConnect API）
- **Anki 端**：`Obsidian-basic` 模型，已注入 Glassmorphism 毛玻璃 CSS
- **AnkiConnect**：本地 8765 端口，配置文件 `%APPDATA%/Anki2/addons21/2055492159/config.json`
