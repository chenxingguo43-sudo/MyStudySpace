...
const ANSWER_OVERRIDES = { 33: 'Б', 39: 'Б' };
const QUESTION_OVERRIDES = {
  ...
  18: { question:'Виталий еще ни разу не был за границей, летом он хочет ...', options:[{key:'А',text:'куда-нибудь'},{key:'Б',text:'куда-то'},{key:'В',text:'кое-куда'},{key:'Г',text:'куда-либо'}] },
  25: { question:'Эта книга ... в свет недавно, но уже стала библиографической редкостью.', options:[{key:'А',text:'пришла'},{key:'Б',text:'дошла'},{key:'В',text:'вошла'},{key:'Г',text:'вышла'}] }
};
...
function buildP5Units({questionsMarkdown,answersMarkdown,expectedRange=[1,50],chapterStart=23}) {
 ...
 for(let n=first;n<=last;n++){const question=QUESTION_OVERRIDES[n]||questions.get(n),answer=answers.get(n);if(!question)throw new Error(`P5 question ${n} is missing after source verification`);if(!answer)throw new Error(`P5 answer ${n} is missing after source verification`);const answerKey=ANSWER_OVERRIDES[n]||answer.answer;exercises.push({id:`P5-Q${pad(n)}`,printedNumber:n,type:'single-choice',question:question.question,options:question.options,answer:answerKey,sourceAnswer:answerKey,sourceEvidence:'PDF-059–PDF-066',sourceExplanation:`原书解析：${answer.sourceExplanation}；译文：${answer.translation}`,referenceExplanation:'参考解析（AI，待复核）：先依据原书给出的语法规则，再结合句子语境判断。',pitfalls:['先识别原书给出的规则，再结合语境判断。'],questionPages:questionPagesForP5(n),answerPages:ANSWER_RULE_PAGES,reviewStatus:'verified'});}
